...
export default function PlayerModal({ server, currentUser, onClose }) {
  ...
  const handleAction = async (action, targetName, extra = '') => {
    ...
    const parsed = parseApiResult(
      await ManagePlayer(server.id, currentUser, action, targetName, extra)
    );
    if (parsed.ok) {
      toast.success('Done.');
  ...
  const isInList = (listKey, name) =>
    lists[listKey]?.some((p) => p.name.toLowerCase() === name.toLowerCase());
                  ...
                  <ScrollArea className="h-full">
                    <div className="px-4 py-4 flex flex-col gap-2">
                      ...
                      {getListForTab(tab).map((p, i) => (
                        <div
                          key={i}
                          className="bg-card px-3.5 py-2.5 rounded-xl flex justify-between items-center border border-border hover:border-primary/30 hover:bg-card/80 transition-all cursor-pointer"
                          onClick={() => setSelectedPlayer(p)}
                        >
                          <div className="flex items-center gap-3 flex-1 min-w-0">
                            <Avatar className="w-9 h-9 rounded-lg shrink-0 ring-1 ring-border">
                              <AvatarImage
                                src={`https://crafatar.com/avatars/${p.uuid || p.name}?size=36&overlay`}
                              />
                            ...
                            <div className="min-w-0">
                              ...
                              {tab === 'HISTORY' && (
                                <div className="flex gap-1 mt-0.5">
                                  {isInList('ops', p.name) && (
                                    <span className="text-[0.6rem] font-bold text-[var(--color-accent-green4)] bg-[var(--color-accent-green4)]/10 px-1.5 py-0.5 rounded">
                                      OP
                                    </span>
                                  )}
                                  {isInList('whitelist', p.name) && (
                                    <span className="text-[0.6rem] font-bold text-[var(--color-accent-blue2)] bg-[var(--color-accent-blue2)]/10 px-1.5 py-0.5 rounded">
                                      WL
                                    </span>
                                  )}
                                  {isInList('banned', p.name) && (
                                    <span className="text-[0.6rem] font-bold text-destructive bg-destructive/10 px-1.5 py-0.5 rounded">
                                      BANNED
                                    </span>
                                  )}
                                </div>
                          ...
                          >
                            ...
                            {tab === 'WHITELIST' && (
                              <Button
                                ...
                                onClick={() => handleAction('whitelist_remove', p.name)}
                              >
                                Remove
                              </Button>
                            )}
                            ...
                            {tab === 'BANNED' && (
                              <Button
                                ...
                                variant="outline"
                                className="h-7 px-2.5 text-xs"
                                onClick={() => handleAction('unban', p.name)}
                              >
                                Pardon
                              </Button>
                            )}
