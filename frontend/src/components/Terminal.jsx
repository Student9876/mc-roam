...
export default function Terminal({ selectedServer }) {
  ...
  const handleSendCommand = async (e) => {
    if (e.key === 'Enter') {
      ...
      const currentUser = sessionStorage.getItem('mc_username') || '';
      const parsed = parseApiResult(
        await SendConsoleCommand(selectedServer.id, currentUser, commandInput)
      );
      if (!parsed.ok) {
        toast.error(parsed.message || 'Failed to send command');
      }
      setCommandInput('');
    }
  };
          ...
          {activeTab === 'terminal' && (
            <div className="border-t border-[var(--color-border-subtle)] px-2 py-2 bg-[var(--color-bg-surface)] flex items-center gap-2">
              ...
              <input
                ...
                placeholder={
                  selectedServer?.lock?.is_running
                    ? 'Type /op, /gamemode, etc...'
                    : 'Server is offline'
                }
...
const LogLine = ({ log }) => {
  ...
  else if (t.includes('[MC]:'))
    color = '#f1c40f'; // Gold
  else if (t.includes('[Playit]:'))
    color = '#4dabf7'; // Blue
  ...
  return (
    <div
      ...
      style={{ color }}
    >
      <span className="text-[#555] mr-2 text-[0.75rem] select-none">{log.time}</span>
      {displayText}
    </div>
  );
};
