...
export default function AdminModal({ server, currentUser, onClose }) {
  ...
  const handleAddAdmin = async () => {
    ...
    setLoading(true);
    try {
      const parsed = parseApiResult(await SetAdmin(server.id, newAdminName.trim(), currentUser));
      if (parsed.ok) {
  ...
  const handleRemoveAdmin = async (adminEntry) => {
    ...
    setLoading(true);
    try {
      const parsed = parseApiResult(await RemoveAdmin(server.id, username, currentUser));
      if (parsed.ok) {
            ...
            <div className="flex gap-3 bg-primary/5 border border-primary/20 rounded-xl p-4 text-sm text-muted-foreground">
              ...
              <div>
                ...
                <p className="mt-1 text-xs leading-relaxed">
                  Admins can modify server settings, world settings, and manage players. Only the
                  server owner can assign or remove admins.
                </p>
              </div>
            </div>
              ...
              <div>
                ...
                <div className="flex gap-2">
                  ...
                  <Button
                    ...
                    disabled={loading || !newAdminName.trim()}
