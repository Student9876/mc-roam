...
export default function WorldModal({ server, currentUser, onClose }) {
  ...
  const handleToggle = async (id) => {
    ...
    try {
      if (id !== 'pvp') {
        const cmdRes = parseApiResult(
          await SendConsoleCommand(server.id, currentUser, `gamerule ${id} ${newVal}`)
        );
        if (!cmdRes.ok) throw new Error(cmdRes.message || 'Failed to run gamerule command');
      }

      const saveRes = parseApiResult(await SaveWorldSetting(server.id, currentUser, id, newVal));
      if (!saveRes.ok) {
        if (saveRes.code === 'LOCAL_PERSIST_FAILED') {
          throw new Error(
            'Saved to database, but local server files could not be updated. Please resync and try again.'
          );
        }
        throw new Error(saveRes.message || 'Failed to save world setting');
      }
    } catch (err) {
  ...
  const handleSelect = async (id, val) => {
    ...
    try {
      if (id === 'difficulty') {
        ...
        if (!cmdRes.ok) throw new Error(cmdRes.message || 'Failed to run difficulty command');
      }

      const saveRes = parseApiResult(await SaveWorldSetting(server.id, currentUser, id, val));
      if (!saveRes.ok) {
        if (saveRes.code === 'LOCAL_PERSIST_FAILED') {
          throw new Error(
            'Saved to database, but local server files could not be updated. Please resync and try again.'
  ...
  const handleIntegerChange = async (id, val) => {
    ...
    try {
      ...
      const saveRes = parseApiResult(await SaveWorldSetting(server.id, currentUser, id, num));
      if (!saveRes.ok) {
        if (saveRes.code === 'LOCAL_PERSIST_FAILED') {
          throw new Error(
            'Saved to database, but local server files could not be updated. Please resync and try again.'
        ...
        >
          {!hasLocalFiles && (
            <div className="mx-6 mt-4 rounded-lg border border-amber-500/40 bg-amber-500/10 px-3 py-2 text-xs text-amber-200">
              Local server files were not found for this instance. World settings are read-only
              until the server files are installed locally.
            </div>
          )}
              ...
              <div className="h-full overflow-y-auto">
                <div className="grid grid-cols-2 gap-3 p-6">
                  {RULE_CATEGORIES[cat].map((rule) => (
                    <div
                      key={rule.id}
                      className="bg-card px-4 py-3.5 rounded-lg flex justify-between items-center border border-border hover:border-muted-foreground/30 transition-colors"
                    >
                      <div>
                      ...
                      {rule.type === 'select' && (
                        <Select
                          value={settings[rule.id]}
                          onValueChange={(v) => handleSelect(rule.id, v)}
                          disabled={!hasLocalFiles}
                        >
