...
export default function AuthPage() {
  ...
  const doLogin = async () => {
    ...
    // Call Go Backend
    const parsed = parseApiResult(await Login(username, password));
  ...
  const doRegister = async () => {
    ...
    const parsed = parseApiResult(await Register(username, password));
    setStatus(parsed.message);
  };
        ...
        <CardContent>
          <div className="flex flex-col gap-3">
            ...
            <div className="flex gap-2 mt-1">
              ...
              <Button onClick={doRegister} variant="outline" className="flex-1">
                Register
              </Button>
            </div>
            {status !== 'Ready' && (
              <p
                className={`text-sm text-center mt-1 ${status.toLowerCase().startsWith('error') ? 'text-destructive' : 'text-[var(--color-accent-blue2)]'}`}
              >
                {status}
              </p>
            )}
