...
export default function Dashboard() {
    ...
    const handleAuthorize = async () => {
        ...
        if (res.startsWith("Error")) toast.error(res);
        else setRcloneConf(res);
    ...
    const handleJoin = async () => {
        ...
        if (res === "Success") toast.success("Joined server!");
    ...
    if (!isSystemReady) {
        return (
            <div className="h-screen flex flex-col items-center justify-center bg-[linear-gradient(135deg,#0f0f0f_0%,#1a1a1a_100%)] text-[var(--color-text-secondary)]">
                ...
                <h2 className="text-3xl font-bold mb-2 text-[var(--color-accent-yellow)]">Setting up Local Cloud</h2>
                ...
                {systemLogs.length > 0 && (
                    <div className="w-[500px] max-h-[300px] bg-[var(--color-bg-deep)] border border-[var(--color-border-subtle)] rounded-lg p-4 overflow-y-auto font-mono text-sm">
                        {systemLogs.map((log, idx) => (
                            <div key={idx} className="mb-1 text-[var(--color-accent-green5)]">{log}</div>
                        ))}
                    </div>
                )}
            </div>
    ...
    const inputCls = "w-full bg-[var(--color-bg-elevated)] border border-[var(--color-border-medium)] rounded text-[var(--color-text-secondary)] p-2.5 text-base outline-none transition-colors focus:border-[var(--color-accent-blue2)]";
        ...
        <div className="flex h-screen bg-[var(--color-bg-root)] text-[var(--color-text-secondary)] overflow-hidden">
            ...
            <div className="flex-1 p-5 overflow-y-auto flex flex-col">
                ...
                {view === "dashboard" && (
                    <div>
                        <div className="flex items-center mb-2.5 justify-between">
                            <h1 className="text-3xl font-bold mb-0 text-[var(--color-accent-yellow)]">My Servers</h1>
                            <Button
                                variant="ghost"
                                size="icon"
                        ...
                        <div className="flex flex-wrap gap-4">
                            {servers.map(server => (
                                <ServerCard
                                    ...
                                    server={{
                                        ...server,
                                        public_address: (server.lock.hosted_by === currentUser && server.lock.is_running && publicAddress) ? publicAddress : null,
                ...
                {view === "create" && (
                    <div className="max-w-[500px]">
                        <h1 className="text-3xl font-bold mb-5 text-[var(--color-accent-yellow)]">Create New Server</h1>

                        {/* STEP INDICATORS */}
                        <div className="flex gap-2.5 mb-5 text-xs items-center">
                            <span style={{ color: createStep >= 1 ? 'var(--color-accent-yellow)' : 'var(--color-border-medium)', fontWeight: createStep === 1 ? 'bold' : 'normal' }}>1. Details</span>
                            <span className="text-[var(--color-border-medium)]">→</span>
                            <span style={{ color: createStep >= 2 ? 'var(--color-accent-yellow)' : 'var(--color-border-medium)', fontWeight: createStep === 2 ? 'bold' : 'normal' }}>2. Cloud</span>
                        </div>
                        ...
                        {createStep === 1 && (
                            <>
                                <div className="mb-4">
                                    <label className="block mb-2 text-sm text-[var(--color-text-sec)]">Server Name</label>
                                    <Input value={newServerName} onChange={e => setNewServerName(e.target.value)} placeholder="e.g. Survival World" />
                                </div>

                                {/* VERSION SELECTION ROW */}
                                <div className="flex gap-4 mb-5">
                                    <div className="flex-1">
                                        <label className="block mb-2 text-sm text-[var(--color-text-sec)]">Server Type</label>
                                        <select className={inputCls} value={selectedType} onChange={(e) => handleTypeChange(e.target.value)} disabled={availableTypes.length === 0}>
                                            {availableTypes.length === 0 && <option>Loading...</option>}
                                            {availableTypes.map(t => <option key={t} value={t}>{t}</option>)}
                                        </select>
                                    </div>
                                    <div className="flex-1">
                                        <label className="block mb-2 text-sm text-[var(--color-text-sec)]">Game Version</label>
                                        <select className={inputCls} value={selectedVersion} onChange={(e) => setSelectedVersion(e.target.value)} disabled={!selectedType}>
                                            {allVersions.filter(v => v.type === selectedType).map(v => (
                                ...
                                <Button onClick={() => setCreateStep(2)} disabled={!newServerName || !selectedVersion} className="w-full">
                                    Next: Cloud Sync →
                                </Button>
                            </>
                        )}
                        ...
                        {createStep === 2 && (
                            <>
                                <div className="mb-4">
                                    <label className="block mb-2 text-sm text-[var(--color-text-sec)]">Google Drive Connection</label>
                                ...
                                <div className="bg-[var(--color-bg-elevated)] p-4 rounded-lg mb-5 text-sm leading-relaxed text-[var(--color-text-sec)]">
                                    <div className="mb-2 text-[var(--color-accent-yellow)] font-bold">🌐 Public Access (Optional)</div>
                                    <p className="m-0">To enable public access for this server, set up your Playit account in <b>Account Settings</b> (sidebar) before hosting.</p>
                                </div>
                                ...
                                <div className="flex gap-2.5">
                                    ...
                                    <Button
                                        ...
                                        onClick={async () => {
                                            ...
                                            if (id.startsWith("Error")) { toast.error(id); return; }
                                            toast.success("Server created successfully!");
                                            setNewServerName(""); setRcloneConf(""); setCreateStep(1); setCreatedServerId(null);
                                            loadServers(); setView("dashboard");
                                        }}
                                    >
                                        ✅ Create Server
                                    </Button>
                ...
                {view === "join" && (
                    <div className="max-w-[500px]">
                        <h1 className="text-3xl font-bold mb-5 text-[var(--color-accent-yellow)]">Join Existing Server</h1>
                        <p className="text-[var(--color-text-dim)] mb-5">Paste the Invite Code shared by your friend.</p>
                        <div className="flex gap-2.5">
                ...
                {view === "account" && (
                    <div className="max-w-[600px]">
                        <h1 className="text-3xl font-bold mb-5 text-[var(--color-accent-yellow)]">Account Settings</h1>

                        <div className="bg-[var(--color-bg-surface)] p-5 rounded-xl mb-5">
                            <h3 className="text-[var(--color-accent-yellow)] mb-4 text-base font-bold">🌐 Public Access (Playit.gg)</h3>
                            ...
                            <div className="bg-[var(--color-bg-elevated)] p-4 rounded-lg mb-5 text-sm leading-relaxed text-[var(--color-text-secondary)] text-left">
                                <div className="mb-2.5 font-bold text-[var(--color-accent-yellow)]">Instructions:</div>
                                <ol className="pl-5 m-0 text-left list-decimal">
                                    <li className="mb-1.5">Click <b>"Launch Setup Terminal"</b> below.</li>
                                    <li className="mb-1.5">Copy the <span className="text-[var(--color-accent-blue2)] font-mono">https://playit.gg/claim/...</span> link, <b>Claim it</b> in your browser.</li>
                                    <li className="mb-1.5">Once it says "Agent Online", you can close the terminal.</li>
                            ...
                            <Button
                                ...
                                onClick={async () => {
                                    ...
                                    if (result.startsWith("Error")) toast.error(result);
                                }}
                            >
                            ...
                            <Button
                                ...
                                onClick={async () => {
                                    ...
                                    if (res === "Success") {
                                        toast.success("Playit config saved! Your tunnel will work on any server you host.");
                                    } else {
                                        toast.error(res);
                                    }
                                }}
                        ...
                        <div className="bg-[var(--color-bg-surface)] p-5 rounded-xl">
                            <h3 className="text-[var(--color-accent-yellow)] mb-2.5 text-base font-bold">👤 User Info</h3>
            ...
            <Dialog open={needsSetup} onOpenChange={(open) => !open && setNeedsSetup(false)}>
                <DialogContent className="max-w-[500px]">
                    ...
                    <DialogFooter>
                        <Button variant="outline" onClick={() => setNeedsSetup(false)}>Cancel</Button>
